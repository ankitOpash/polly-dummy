...
const TreeMenuItem: React.FC<TreeMenuItemProps> = ({
  ...
  // const [isOpen, setOpen] = useState<boolean>(active); // WHEN ADD OTHERS PARENT CATEGORY THEN CHANGE THIS //

  const [isOpen, setOpen] = useState<boolean>(true);

  // WHEN ADD OTHERS PARENT CATEGORY THEN CHANGE THIS //
  // useEffect(() => {
  //   setOpen(isActive);
  // }, [isActive]);
  ...
  return (
    <>
      ...
      >
        <button
          className={cn(
            'flex w-full items-center py-2 font-semibold text-body-dark outline-none transition-all ease-in-expo  focus:text-accent focus:outline-0 focus:ring-0 text-left rtl:text-right',
            isOpen ? 'text-accent' : 'text-body-dark',
            className ? className : 'text-sm'
          )}
        >
          {icon && (
            <span className="flex h-5 w-5 items-center justify-center mr-4 rtl:ml-4">
              {getIcon({
          ...
          <span>{name}</span>
          <span className="ml-auto mr-4 rtl:ml-4 rtl:mr-auto">
            {expandIcon}
          </span>
        </button>
      </motion.li>
        ...
        {Array.isArray(items) && isOpen ? (
          <li>
            <motion.ul
              ...
              className="text-xs ml-4 rtl:mr-4"
            >
              {items.map((currentItem) => {
                ...
                return (
                  <TreeMenuItem
                    ...
                    item={currentItem}
                    depth={childDepth}
                    className={cn('text-sm text-body ml-5 rtl:mr-5')}
                  />
                );
              })}
            </motion.ul>
