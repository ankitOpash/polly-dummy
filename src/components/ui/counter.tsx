...
const variantClasses = {
  helium:
    'w-7 h-18 sm:w-20 sm:h-7 md:h-9 md:w-24 bg-accent flex-col-reverse sm:flex-row absolute sm:static bottom-3 right-3 rtl:left-3 sm:bottom-0 sm:right-0 sm:left-0 text-light rounded',
  ...
  single:
    'order-5 sm:order-4 w-9 sm:w-24 h-24 sm:h-10 bg-accent text-light rounded-full flex-col-reverse sm:flex-row absolute sm:relative bottom-0 sm:bottom-auto right-0 rtl:left-0 sm:right-auto sm:left-auto',
  details:
    'order-5 sm:order-4 w-full sm:w-24 h-10 bg-accent text-light rounded-full',
  pillVertical:
    'flex-col-reverse items-center w-8 bg-gray-100 text-heading rounded-full',
  cart:
    'xl:w-3/5 sm:w-full w-[110px] h-7 md:h-9 items-center w-8 h-24 bg-gray-100 text-heading rounded-full',
      ...
      <button
        ...
        className={cn(
          'cursor-pointer p-2 transition-colors duration-200 hover:bg-accent-hover focus:outline-0',
          {
            ...
            'px-5': variant === 'big',
            'border border-gray-300 px-5 hover:border-accent hover:!bg-transparent rounded-l rtl:rounded-r':
              variant === 'bordered',
      ...
      <button
        ...
        className={cn(
          'cursor-pointer p-2 transition-colors duration-200 hover:bg-accent-hover focus:outline-0',
          {
            'px-3 py-3 sm:px-2': variant === 'single',
            'px-5': variant === 'big',
            'border border-gray-300 px-5 hover:border-accent hover:!bg-transparent hover:!text-accent rounded-r rtl:rounded-l':
              variant === 'bordered',
            'hover:!bg-gray-100': variant === 'pillVertical',
          }
        )}
