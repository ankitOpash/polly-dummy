...
const AddToCartBtn: React.FC<Props> = ({ variant, onClick, disabled }) => {
  ...
  switch (variant) {
    case 'neon':
      return (
        <button onClick={onClick} disabled={disabled} className='flex w-full items-center justify-center rounded bg-accent py-4 px-5 text-sm font-light text-light transition-colors duration-300 hover:bg-accent-hover focus:bg-accent-hover focus:outline-0 lg:text-base'>Add to Selection</button>
      );
    case 'argon':
      return (
        <button
    ...
    case 'single':
      return (
        ...
        >
          <CartIcon className="h-4 w-4 mr-2.5 rtl:ml-2.5" />
          <span>Cart</span>
        </button>
      );
    case 'big':
