...
const HeaderMinimal = ({ layout }: { layout: string }) => {
  ...
  return (
    <header className={cn('site-header-with-search h-14 md:h-16 lg:h-22')}>
      <div
        className={cn(
          'fixed z-50 flex h-14 w-full items-center justify-between border-b border-border-200 bg-light px-4 py-5  shadow-sm transition-transform duration-300 md:h-16 lg:h-22 lg:pl-12 lg:pr-8 rtl:lg:pr-12 rtl:lg:pl-8',
          {
      ...
      >
        <div className="flex w-full items-center lg:w-auto">
          ...
          <Logo className={`${!isMultilangEnable ? 'mx-auto lg:mx-0' : 'ml-0 rtl:mr-0'}`} />

          <ul className="hidden shrink-0 items-center space-x-7 ml-10 mr-auto rtl:mr-10 rtl:ml-auto rtl:space-x-reverse lg:flex 2xl:space-x-10">
            <StaticMenu />
        ...
        {displayMobileHeaderSearch && (
          <div className="absolute top-0 block h-full w-full bg-light px-5 pt-1.5 left-0 rtl:right-0 md:pt-2 lg:hidden">
            <SearchWithSuggestion
              label={'grocery search at header'}
              variant="minimal"
              seeMore={true}
            />
          </div>
