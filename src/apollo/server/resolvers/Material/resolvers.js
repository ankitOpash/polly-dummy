...
export const resolvers = {
  Query: {
    getAllMaterials: combineResolvers(hasPermission('Materials', 'view'), async (_, { input }, { user }) => {
      try {
        let filter = JSON.parse(input?.filter || '{}')

        const filterText = GeneralHelpers.filterSearchQuery(input?.search, 'MaterialTbl')
        ...
        const returnData = await models.Material.aggregate(
    ...
    getMaterialDetailById: combineResolvers(hasPermission('Materials', 'view'), async (_, { id }, { user }) => {
      try {
        let material = await models.Material?.findById(id)
          .populate('tags')
          .populate('grade')
          .populate({
            path: 'parentCategory'
  ...
  Mutation: {
    createMaterial: combineResolvers(hasPermission('Materials', 'create'), async (_, { data }, { user }) => {
      try {
        const isMaterialExists = await models.Material.findOne({
          name: capitalizeFirstLetter(data.name),
          company: user.company,
          isDeleted: false
        })

        const isSkuExist = await models.Material.findOne({
          sku: data.sku.trim(),
          company: user.company,
          isDeleted: false
        })
        ...
        if (isSkuExist) {
          ...
          let newMaterial = await models.Material.create({
            name: capitalizeFirstLetter(data.name),
            sku: data.sku,
    ...
    deleteMaterials: combineResolvers(hasPermission('Materials', 'delete'), async (root, { id }, { user }) => {
      try {
        const material = await models.Material.find({ _id: { $in: id }, company: user.company, isDeleted: false })
        if (!material) {
          throw new CustomError(MATERIAL_ERROR.MATERIAL_NOT_FOUND, 405)
        } else {
          let result = await models.Material.updateMany(
    ...
    updateMaterials: combineResolvers(hasPermission('Materials', 'edit'), async (root, { id, data }, { user }) => {
      try {
        const isMaterialExists = await models.Material.findOne({ _id: id, company: user.company, isDeleted: false })
        if (!isMaterialExists) {
          throw new CustomError(MATERIAL_ERROR.MATERIAL_NOT_FOUND, 405)
        }
        if (isMaterialExists.name !== data.name) {
          const materialWithSameName = await models.Material.findOne({
            _id: { $ne: isMaterialExists._id },
            name: data.name,
            company: user.company,
            isDeleted: false
          })
        ...
        if (isMaterialExists.sku !== data.sku) {
          const skuwithSameName = await models.Material.findOne({
            _id: { $ne: isMaterialExists._id },
            sku: data.sku,
            company: user.company,
            isDeleted: false
        ...
        await models.Material.updateOne(
          { _id: isMaterialExists._id },
          {
            $set: {
              name: data.name.trim(),
              sku: data.sku,
        ...
        const returnUpdatedData = await models.Material.findOne({ _id: isMaterialExists._id }).lean().exec()
        let imagesToBeDeleted = lodash.difference(isMaterialExists.images || [], returnUpdatedData.images || [])
    ...
    updateMaterialSettings: combineResolvers(
      ...
      async (root, { id, data }, { user }) => {
        try {
          ...
          const returnData = await models.Material.findOne({ _id: isMaterialExists._id }).lean().exec()

          let imagesToBeDeleted = lodash.difference(isMaterialExists.images || [], returnData.images || [])
          if (imagesToBeDeleted && imagesToBeDeleted.length) {
            let fileObjects = []
            imagesToBeDeleted.forEach(img => {
              const { filepath, filename } = extractFilePath(img)
